...
program
  ...
  .option("-s, --storeDir  [path]", "Specify directory where store is defined.")
  .option(
    "-d, --distDir [path]",
...
let config;
try {
  const configFileName = program.config || "vuex-guardian.config.js";
  const configFilePath = path.resolve(configFileName);
  config = require(configFilePath);
} catch {
  config = require("../dist/config").config;
} finally {
  require("../dist").run({ build: program.build, ...config });
}
